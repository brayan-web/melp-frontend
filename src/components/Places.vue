<template>
  <div class="flex">
    <div class="w-1/3">
      <div class="w-full text-center mt-10">
        ...
        <div class="mx-auto w-[90%]">
            <label for="radius" class="text-left block mb-2 text-sm font-medium text-gray-300 dark:text-white">Enter radius</label>
            <input v-model="radius" type="number" id="radius" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  block w-full p-2.5 " placeholder="radius" required>
        </div>
        <div class="z-10 w-[90%] mx-auto mt-5">
          <label for="raiting" class="text-left block mb-2 text-sm font-medium text-gray-300 dark:text-white">Select a raiting</label>
    ...
    <div class="w-2/3">
      <RecomendationsPlace  @get-coordinates="getCoordinates"/>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const fetchPlaces = async() => {
      ...
      if(radius.value !== null && selectedPerson.value.name !== ''){
           try {
